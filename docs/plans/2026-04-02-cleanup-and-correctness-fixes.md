...
- [ ] remove `SYNTHETIC_LINEAR_FIELD` constant (line 87)
- [ ] remove `mark_synthetic_linear_record()` function (lines 133-136)
- [ ] remove `is_synthetic_linear_record()` function (lines 139-143)
- [ ] remove test `test_mark_synthetic_linear_record` (lines 280-284)
- [ ] remove test `test_is_synthetic_linear_record_false_by_default` (lines 287-290)
- [ ] run tests — must pass before next task

### Task 2: Remove synthetic filters from recent.rs
- [ ] remove all `if session::is_synthetic_linear_record(&json) { continue; }` occurrences (lines 122, 186, 293, 399, 531)
- [ ] simplify combined condition at line 344 — keep only the record type check
- [ ] remove test `test_extract_summary_returns_none_for_synthetic_linear_bootstrap_only`
- [ ] remove test `test_extract_summary_keeps_resumed_synthetic_branch_visible`
- [ ] run tests — must pass before next task

### Task 3: Remove synthetic filter from search/message.rs
- [ ] remove `if session::is_synthetic_linear_record(&json) { return None; }` (lines 28-30)
- [ ] verify `use crate::session;` import is still used (yes — `extract_record_type` etc.)
- [ ] remove test `test_skip_synthetic_linear_message`
- [ ] run tests — must pass before next task

### Task 4: Remove linearization code from resume/launcher.rs
- [ ] remove `SYNTHETIC_SOURCE_PATH_FIELD` constant (line 18, `#[cfg(test)]`)
- [ ] remove `SessionAnalysis.is_linear` field (line 29) and all `#[cfg(test)]` computation blocks: variables (lines 44-48), UUID tracking (lines 57-61, 74-76), chain walk (lines 107-123), struct literal field (lines 131-132)
...
- [ ] remove tests: `test_analyze_session_treats_interleaved_metadata_as_linear`, `test_analyze_session_still_detects_real_branch_with_metadata_nodes`
- [ ] remove all `test_create_linear_session_*` tests (6 tests)
- [ ] remove all `test_cleanup_legacy_synthetic_sessions_*` tests (3 tests)
- [ ] run tests — must pass before next task
...
- [ ] in `build_chain_from_tip`: after parsing json, skip records where `isSidechain` is true — `json.get("isSidechain").and_then(|v| v.as_bool()).unwrap_or(false)`
- [ ] in `latest_tip_uuid`: same isSidechain filter
- [ ] in `create_fork`: same isSidechain filter when building uuid_to_parent map
- [ ] write test `test_build_chain_ignores_sidechain_records` — sidechain entry at end should not become tip
...
- [ ] in `build_chain_from_tip`: detect `{"type":"system","subtype":"compact_boundary"}`, on match clear `uuid_to_parent` and `last_uuid` — only post-boundary records matter
- [ ] in `latest_tip_uuid`: same compact_boundary reset
- [ ] in `create_fork`: same compact_boundary reset (clear uuid_to_parent, skip pre-boundary lines)
- [ ] write test `test_build_chain_resets_on_compact_boundary` — pre-boundary UUIDs not in chain
...
- [ ] in `create_fork`: change `None =>` arm (lines without uuid) from including as-is to skipping
- [ ] write test `test_create_fork_skips_metadata_without_uuid` — verify summary/snapshot lines not in output
- [ ] update existing test `test_create_fork_from_non_latest_branch` if it asserts metadata presence
- [ ] run tests — must pass before next task
...
- [ ] rewrite `build_latest_chain()`: compute `parent_uuids` set from all nodes, find terminals (uuid not in parent_uuids), filter to user/assistant with `!isSidechain`, pick the one with latest timestamp as tip
- [ ] add `isSidechain` check when collecting nodes in `from_file()` — read `json.get("isSidechain")` and store in `DagNode`
- [ ] write test `test_latest_chain_ignores_trailing_metadata` — metadata uuid at end should not become tip
- [ ] write test `test_latest_chain_ignores_sidechain_leaf` — isSidechain leaf should not become tip
- [ ] write test `test_latest_chain_picks_latest_terminal` — with multiple terminal branches, picks latest timestamp
- [ ] run tests — must pass before next task
...
- [ ] in `from_file()`: after reading `parent_uuid`, if it's `None`, check `json.get("logicalParentUuid")` and use it as fallback
- [ ] in `find_displayable_parent()`: same fallback (already walks parent_uuid, will now traverse through logical links)
- [ ] write test `test_logical_parent_uuid_bridges_compact_boundary` — compact_boundary with logicalParentUuid should connect pre/post-compact chains
- [ ] run tests — must pass before next task
...
- [ ] verify `find_displayable_parent()` correctly walks through progress nodes (read code at tree/mod.rs:281-315)
- [ ] write test `test_display_graph_bridges_progress_nodes` — user(u1) -> progress(p1) -> user(u2) should show u1 as parent of u2 in display
- [ ] if bridge doesn't work, add explicit progress bridging in `from_file()` similar to Claude Code's approach
- [ ] run tests — must pass before next task
...
- [ ] verify system nodes participate correctly in the DAG (they should be bridges, not displayed)
- [ ] write test `test_system_nodes_bridge_correctly` — user(u1) -> system(s1) -> user(u2), display graph should show u1 as parent of u2
- [ ] write test `test_attachment_nodes_bridge_correctly` — similar test with attachment type
- [ ] run tests — must pass before next task
...
- [ ] test manually: run `claude --resume <session-id> --fork-session` and verify behavior
- [ ] test: does it work with a specific message? (we resume from a non-latest branch tip)
- [ ] document findings as comments in `resume/mod.rs`
- [ ] if --fork-session can replace our fork: add a TODO/note for future simplification (separate plan)
- [ ] if it can't: document WHY in comments (e.g., "Claude's --fork-session only forks from latest leaf, not arbitrary branch tips")
- [ ] run tests — must pass before next task
