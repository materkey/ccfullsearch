...
### New Features
- Opencode sessions support — discover and search Opencode SQLite databases (`opencode.db` and channel-suffixed variants like `opencode-dev.db`) alongside Claude Code CLI, Claude Desktop, and Codex rollouts.

### Changed
- Consolidate `fmt` / `clippy` / `test` behind a single `make check` target — single entry point that CI runs; individual targets (`make fmt` / `clippy` / `test`) remain callable.
- Add `CONTRIBUTING.md` and a minimal PR template (Testing + Notes).
