...
1. **Search**: spawn-per-request with cooperative + forced cancellation. `SearchHandle { seq, cancel }` in `src/tui/state.rs` is the single source of truth for in-flight state; `is_searching()` is a derived predicate (`self.search.current.is_some()`), not a stored flag.
   - **Start**: User types → 300 ms debounce → `start_search` increments `search_seq`, cancels the prior handle via `SearchHandle.cancel.store(true)`, and installs a fresh handle.
   - **Worker thread**: per-request thread streams `rg --json --glob="*.jsonl"` via `Command::spawn()` + `BufReader::lines()`, checking `cancel.load()` on every line. On cancel: `child.kill()` + `child.wait()` + `Err("cancelled")`. A dedicated stderr-drain thread keeps the pipe buffer from filling and surfaces stderr text in any error message. On completion the thread sends `BackgroundSearchResult` only if `!cancel.load()`, suppressing zombie sends from superseded requests.
   - **Tick**: `App.tick()` polls `result_rx` and routes by `seq == current.seq` (stale results are dropped). On match: clears `current`, parses matches into `Message` (Full mode for content, TextOnly mode for clean preview), post-filters so the query actually appears in message *content* (not metadata), groups by `session_id`, sorts by timestamp desc.
   - **Truncation**: if any file hit the per-file match limit (1000), `truncated` is flagged and a warning surfaces in the status bar.
